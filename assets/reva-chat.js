(() => {
  ...
  const history = [];
  const add = (role, text) => { const p=document.createElement('p'); p.className=`reva-message reva-message--${role}`; String(text).split(/(https?:\/\/[^\s]+)/g).forEach((part) => { if(/^https?:\/\//.test(part)){const a=document.createElement('a');a.href=part;a.target='_blank';a.rel='noopener';a.textContent='Buka berita ↗';p.appendChild(a)}else{p.appendChild(document.createTextNode(part))} }); messages.appendChild(p); messages.scrollTop=messages.scrollHeight; };
